...
class Player:
    ...
    def __init__(self,
        ...
        if value_function == "random":
            self.value_function = initialize_random_value_function()
        else:
        ...
        for game_num in range(1, num_games + 1):
            ...
            while not game_over:
                ...
                if move_counter >= 2:
                    ...
                    player.update_value_function(state=boards[-3], next_state=boards[-1])
                if game_over:
                    if winner == 0:
                        boards.append("draw_state")
                        player.update_value_function(state=boards[-2], next_state=boards[-1])
                    else:
                        boards.append("loser_state")
                    player_2 = self.player_b if move_counter % 2 == a_begins else self.player_a
                    player_2.update_value_function(state=boards[-3], next_state=boards[-1])
                    player.num_games += 1
...
class Competition(Dojo):
    ...
    """
    def __init__(self, value_function_a="random", value_function_b="random"):
        """
...
def initialize_random_value_function():
    """
    ...
    """
    values = 1 + np.random.uniform(0, 0.01, len(STATES))
