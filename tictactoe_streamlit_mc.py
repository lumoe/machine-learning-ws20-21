...
RL_agent = Player('a_optimal_MC', 0., 0., 0., 0.)
RL_agent.sign = 1
...
def show():
    st.write(
        ...
        This agent was trained using Monte Carlo Learning
        """
    )
    st.write("")
    ...
    if winner != None:
        if winner == -1:
            st.success(f"You won against our agent! :party:")
        elif winner == 1:
            st.error(f"Our agent is better than yours ")
